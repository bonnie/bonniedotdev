...
test.skip('mount when localstorage has login info already', async () => {
  ...
  localStorage.__STORE__.bonniedotdev_user =
    '{"id":1,"username":"loggedinUser"}';

  // render login page
  const userScreen = renderWithRouterAndProvider(<App />, {
    initialRouterEntries: ['/login'],
  });

  // should show welcome message for logged-in user
  const welcomeHeader = await userScreen.findByRole('heading', {
    name: /welcome loggedinUser/i,
  });
  expect(welcomeHeader).toBeInTheDocument();
});
