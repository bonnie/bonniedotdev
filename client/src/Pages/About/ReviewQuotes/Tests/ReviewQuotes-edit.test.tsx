...
test('create new review quote and save', async () => {
  ...
  const newReviewQuoteScreen = renderWithProvider(<ReviewQuotes />, initialState);

  // wait until review quote edit buttons appear so we know data is loaded
  const editButtons = await newReviewQuoteScreen.findAllByRole('button', { name: /edit review quote/i });
  expect(editButtons.length).toBe(5);

  // find and click add button
  const addButton = newReviewQuoteScreen.getByRole('button', { name: /add review quote/i });
  userEvent.click(addButton);

  // wait for new review quote modal to show
  const reviewQuoteModalTitle = await newReviewQuoteScreen.findByRole('heading', { name: /add review quote/i });
  ...
  const courseSelectOption = await newReviewQuoteScreen.findByRole('option', { name: 'Course 2' });
  userEvent.click(courseSelectOption);

  // click button to save new review quote
  const saveButton = newReviewQuoteScreen.getByRole('button', { name: /save/i });
...
test('update a review quote', async () => {
  // render with pre-defined state for user
  const initialState = { user: { id: 1, username: 'sheila' } };
  const editReviewQuoteScreen = renderWithProvider(<ReviewQuotes />, initialState);

  // wait until review quote edit buttons appear so we know data is loaded
  const editButtons = await editReviewQuoteScreen.findAllByRole('button', { name: /edit review quote/i });
  expect(editButtons.length).toBe(5);
  ...
  const modalTitle = editReviewQuoteScreen.getByRole('heading', { name: /edit review quote/i });
  expect(modalTitle).toBeVisible();

  // update content and confirm update
  const bodyField = editReviewQuoteScreen.getByRole('textbox', { name: 'body' });
  userEvent.clear(bodyField);
  userEvent.type(bodyField, 'Just going to dip my paw in your coffee and do a taste test - oh never mind i forgot i don\'t like coffee - you can have that back now');

  // click save button
  const saveButton = editReviewQuoteScreen.getByRole('button', { name: /save/i });
  userEvent.click(saveButton);
...
test('delete a talk', async () => {
  ...
  const deleteButtons = await deleteTalkScreen.findAllByRole('button', { name: /delete review quote/i });
