...
describe('new courses', () => {
  ...
  test('flow to add and delete new course', async () => {
    ...
    const courseForms = await addButtonScreen.findAllByRole('form', {
      name: /course \d+/i,
    });
    ...
    const courseFormsPlusOne = await addButtonScreen.findAllByRole('form', {
      name: /course \d/i,
    });
    expect(courseFormsPlusOne.length).toBe(3);

    // 'add' button should have disappeared
    const notAddButton = addButtonScreen.queryByRole('button', {
      name: /add course/i,
    });
    ...
    const reappearedAddButton = await addButtonScreen.findByRole('button', {
      name: /add course/i,
    });
    expect(reappearedAddButton).toBeInTheDocument();

    // check that course no longer appears
    const courseFormsMinusOne = addButtonScreen.getAllByRole('form', {
      name: /course \d/i,
    });
